...
int main(int argc, char *argv[])
{
    ...
    else if (userDecision == "S")
    {
        ...
        //call quicksort for each individual size. 
        //send in one row at a time, nest a loop around the time clock 
        startTime = high_resolution_clock::now();
        //row needs to be incremented each time quicksirt is called
        quickSort(0, data, 0, size);
        endTime = high_resolution_clock::now();
        
        //cpu time for computation of each dataset
        cputime = duration_cast<microseconds>(endTime - startTime).count();
        ...
        cout << "I" << "\t" << comparisons << "\t" << cputime << endl; 
...
void quickSort(int row, int ary[][500000], int low, int high)
{
    if (low < high) {
 
        int q = partition_random(row, ary, low, high);
...
// Random Pivot
int partition_random(int row, int ary[][500000], int low, int high)
{
    ...
    return partition(row, ary, low, high);
}


int partition(int row, int ary[][500000], int low, int high)
{
